...
import { ensureBootstrapData, initialAdminPasswordFile } from './db/bootstrap.js';
...
// 초기 비밀번호 파일 자동 삭제는 실서버에서만 켠다(opt-in) — admin 비밀번호 변경·재설정 시 삭제
const initialPwFile = initialAdminPasswordFile(dataDir);
app.use('/api/auth', createAuthRouter(db, { initialPwFile }));
...
app.use('/api/admin/users', requireAdmin(db), createUsersRouter(db, { initialPwFile })); // 사용자 계정 관리(admin) — 더 구체적 경로 먼저
...
/** 내부망 IPv4 주소 열거 — 다른 PC에서 접속할 주소 안내용 */
...
const server = app.listen(config.port, () => {
  ...
  console.log(`[우수검사실 인증심사 웹 바인더] v${pkg.version}`);
  console.log(`  이 PC에서 열기:        http://localhost:${config.port}`);
  const lan = lanAddresses();
  if (lan.length > 0) {
    for (const ip of lan) {
      console.log(`  같은 내부망에서 열기:  http://${ip}:${config.port}`);
    }
  } else {
    const ipHint = process.platform === 'win32' ? 'ipconfig' : 'hostname -I';
    console.log(`  (내부망 주소를 찾지 못했습니다 — 다른 PC에서 접속하려면 ${ipHint} 로 IP 주소를 확인하세요.)`);
  }
  console.log(`[db] ${dbPath} 연결됨 (WAL, foreign_keys=ON, user_version=${userVersion})`);
  // 초기 비밀번호는 최초 부팅 때 1회만 생성·표시된다 — 파일이 남아 있는 동안은 매 부팅마다 위치를 안내
  if (fs.existsSync(initialPwFile)) {
    console.log(`[안내] 관리자(admin) 초기 비밀번호: ${initialPwFile} 파일 참고 — 로그인 후 비밀번호를 바꾸고 파일을 지우세요.`);
  }

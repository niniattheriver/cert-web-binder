...
export interface FullBundleResult extends BundleResult {
  /** ZIP에 담긴 원본 파일 수(스냅샷·manifest 제외) */
  fileCount: number;
  /** 원본이 없어 담지 못한 파일 수 */
  missingCount: number;
}

const FULL_ZIP_LIMIT = 0xffffffff; // ZIP64 미지원 — 4GB 한계(zip-stream.ts와 동일 전제)

/**
 * 전체 백업(PDF 포함): VACUUM 스냅샷 + data/files/ 원본 전부 + manifest.json 을 한 ZIP으로.
 * ZIP 내부를 data/ 폴더 구조 그대로(data/app.db, data/files/…) 담아
 * "압축 해제 → data 폴더 배치"만으로 복원되게 한다(복원 절차는 운영런북).
 * 큰 PDF는 스트리밍(STORE)으로 담아 메모리를 쓰지 않고, async로 이벤트 루프도 막지 않는다.
 */
export async function createFullBackupBundle(
  db: Database.Database,
  backupsDir: string,
  filesDir: string,
  now: Date = new Date(),
  ...
  fs.mkdirSync(backupsDir, { recursive: true });
  const label = timestampLabel(now);
  const tmpSnapshot = path.join(backupsDir, `.tmp-full-${label}-${process.pid}.db`);
  const tmpZip = path.join(backupsDir, `.tmp-full-backup-${label}-${process.pid}.zip`);
  vacuumInto(db, tmpSnapshot);
  try {
    const snapshotBytes = fs.statSync(tmpSnapshot).size;
    const manifest = buildFileManifest(db, filesDir, now);

    const estimated = snapshotBytes + manifest.totalBytes + 1024 * 1024;
    if (estimated >= FULL_ZIP_LIMIT) {
      throw new Error(
        '전체 백업이 ZIP 형식 한계(4GB)를 넘어 만들 수 없습니다. data 폴더 복사 방식을 사용하세요.',
      );
    }
    try {
      const s = fs.statfsSync(backupsDir);
      const freeBytes = s.bavail * s.bsize;
      if (Number.isFinite(freeBytes) && freeBytes > 0 && freeBytes < estimated * 1.05) {
        throw new Error('저장 공간이 부족해 전체 백업을 만들 수 없습니다. 오래된 백업을 정리해 주세요.');
      }
    } catch (err) {
      if ((err as Error).message.includes('저장 공간')) throw err;
      // statfs 미지원 플랫폼 — 여유 공간 검사만 생략
    }

    const writer = await createZipFileWriter(tmpZip);
    let fileCount = 0;
    let zipBytes = 0;
    try {
      await writer.addFile('data/app.db', tmpSnapshot, now);
      const seen = new Set<string>();
      for (const row of manifest.files) {
        if (!row.exists) continue;
        const rel = row.relPath.split(path.sep).join('/');
        if (seen.has(rel)) continue;
        seen.add(rel);
        await writer.addFile(`data/files/${rel}`, path.join(filesDir, row.relPath), now);
        fileCount++;
      }
      ...
      ({ zipBytes } = await writer.finalize());
    } catch (err) {
      await writer.abort(); // finalize 실패 포함 — 핸들을 닫아야 finally의 임시 ZIP 삭제가 가능(Windows)
      throw err;
    }

    const zipFile = `full-backup-${label}.zip`;
    const zipPath = path.join(backupsDir, zipFile);

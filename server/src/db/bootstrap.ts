...
/** 초기 admin 비밀번호 안내 파일 경로 — 생성(여기)·안내(/api/me, index.ts)·자동 삭제(auth/users) 공용 */
export function initialAdminPasswordFile(dataDir: string): string {
  return path.join(dataDir, 'initial-admin-password.txt');
}
...
export function ensureBootstrapData(db: Database.Database, dataDir: string): void {
  ...
  if (userCount === 0) {
    ...
    // 안내 파일을 계정 생성보다 먼저 기록한다 — 첫 부팅 도중 창을 닫아도(프로세스 중단)
    // 계정만 생기고 비밀번호를 알 길이 없어지는 순서가 되지 않게. 파일만 남고 계정 생성 전에
    // 중단된 경우는 다음 부팅이 userCount=0 을 보고 처음부터 다시 시드하며 파일을 덮어쓴다.
    if (!envPw) {
      const pwFile = initialAdminPasswordFile(dataDir);
      fs.writeFileSync(
        pwFile,
        `admin 초기 비밀번호: ${password}\n최초 로그인 후 비밀번호를 변경하세요 — 변경하면 이 파일은 자동 삭제됩니다.\n`,
        { mode: 0o600 },
      );

...
import { initialAdminPasswordFile } from '../db/bootstrap.js';
...
/** 라우터 옵션 — initialPwFile 을 명시 주입한 경우에만 초기 비밀번호 파일 자동 삭제가 동작한다(opt-in).
 *  기본값을 두지 않는 이유: 테스트가 admin 비밀번호 변경을 실호출하므로, 기본 경로 삭제로 구현하면
 *  테스트 실행이 개발 장비의 실파일을 지운다. 실서버(index.ts)에서만 경로를 주입한다. */
export interface AuthRouterOpts {
  initialPwFile?: string;
}

/** admin 계정의 비밀번호가 새로 설정됐을 때 초기 비밀번호 안내 파일을 지운다(더 이상 유효하지 않음). */
export function deleteInitialAdminPasswordFile(pwFile: string): void {
  try {
    if (fs.existsSync(pwFile)) {
      fs.rmSync(pwFile);
      console.log('[안내] 초기 비밀번호 파일을 삭제했습니다 (비밀번호 변경 완료).');
    }
  } catch {
    console.warn(`[안내] 초기 비밀번호 파일 삭제에 실패했습니다 — 직접 삭제해 주세요: ${pwFile}`);
  }
}
...
export function createAuthRouter(db: Database.Database, opts: AuthRouterOpts = {}): Router {
  const router = Router();
  ...
  router.post('/change-password', requireAuth(db), (req, res) => {
    ...
    // 부트스트랩 admin 계정의 비밀번호가 바뀌면 초기 비밀번호 안내 파일은 더 이상 유효하지 않다
    if (opts.initialPwFile && me.username === 'admin') {
      deleteInitialAdminPasswordFile(opts.initialPwFile);
    }
    logChange(db, {
      actorId: me.id,
      actorKind: 'user',
...
export function createMeHandler(db: Database.Database): RequestHandler {
  const pwFile = initialAdminPasswordFile(dataDir);
  return (req, res) => {

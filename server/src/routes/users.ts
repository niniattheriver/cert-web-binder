...
import {
  deleteInitialAdminPasswordFile,
  destroyUserSessions,
  type AuthRouterOpts,
} from './auth.js';
...
/** 사용 가능한 admin 수 — 활성이면서 만료되지 않은 계정만 (자기 잠금·마지막 admin 보호 판정용) */
function activeAdminCount(db: Database.Database): number {
  return (
    db
      .prepare(
        `SELECT COUNT(*) AS n FROM user
         WHERE role = 'admin' AND active = 1 AND (expires_at IS NULL OR expires_at > ?)`,
      )
...
export function createUsersRouter(db: Database.Database, opts: AuthRouterOpts = {}): Router {
  const router = Router();
  ...
  router.patch('/:id', (req, res) => {
    ...
    // 만료일 설정도 잠금과 같은 효과 — 자기 계정에는 설정 금지(해제(null)는 허용)
    if (isSelf && p.expiresAt != null) {
      res.status(409).json({ error: 'self_lockout', details: '자기 계정에는 만료일을 설정할 수 없습니다.' });
      return;
    }
    // 마지막 사용 가능 admin 보호: 강등/비활성/만료 설정으로 사용 가능한 admin이 0이 되면 차단
    const nowIso = new Date().toISOString();
    const curUsable = cur.active === 1 && (cur.expires_at == null || cur.expires_at > nowIso);
    const willDropAdmin =
      cur.role === 'admin' &&
      curUsable &&
      ((p.role != null && p.role !== 'admin') ||
        p.active === false ||
        (p.expiresAt != null && p.expiresAt <= nowIso));
  ...
  router.post('/:id/reset-password', (req, res) => {
    ...
    // 부트스트랩 admin 계정이 재설정되면 초기 비밀번호 안내 파일은 더 이상 유효하지 않다(opt-in)
    if (opts.initialPwFile && cur.username === 'admin') {

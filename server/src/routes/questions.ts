...
function fullQuestionPayload(db: Database.Database, id: number): Record<string, unknown> | null {
  ...
  if (!row) return null;
  const cat = db
    .prepare('SELECT id, code, name, cycle_id FROM category WHERE id = ?')
    .get(row.category_id) as CategoryRow & { cycle_id: number };
  // breadcrumb 이 연도 대시보드(/y/연도)로 링크할 수 있게 주기 정보를 함께 내려준다 (v1.5.4)
  const cycle =
    (db.prepare('SELECT id, name, year FROM cycle WHERE id = ?').get(cat.cycle_id) as
...
export function createQuestionsRouter(db: Database.Database): Router {
  ...
  router.get('/categories/:id/questions', auth, (req, res) => {
    ...
    const cycle =
      (db.prepare('SELECT id, name, year FROM cycle WHERE id = ?').get(category.cycle_id) as
        | { id: number; name: string; year: number | null }
        | undefined) ?? null;
    res.json({
      category,

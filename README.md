...
설치는 딱 두 단계입니다: **① Node.js 설치(최초 1회) → ② 프로그램 zip 내려받아 더블클릭**.

### ① Node.js 설치 (최초 1회)

Node.js는 이 프로그램을 움직이는 엔진입니다. 무료이고, 한 번만 설치하면 됩니다.
(이미 설치된 PC라면 이 단계는 건너뛰세요. 최신 LTS를 받으면 되고, 이미 깔린 버전을 쓰는 경우
20.19 / 22.12 이상이어야 합니다.)

1. 인터넷이 되는 상태에서 **https://nodejs.org/ko** 에 접속해, 가운데 **초록색 [Get Node.js®] 버튼**을 누릅니다.

   ![Node.js 홈 — 초록 버튼](docs/img/install-nodejs.png)

2. 다운로드 페이지가 열리면 **화면 아래쪽**의 초록 버튼 **[Windows 설치 프로그램 (.msi)]** 을 눌러 파일을 받습니다.
   위쪽의 검은 명령어 상자는 개발자용이니 **무시하셔도 됩니다.**

   ![다운로드 페이지 — Windows 설치 프로그램](docs/img/install-nodejs-msi.png)

3. 내려받은 파일을 더블클릭하고, 설치 창에서 아무것도 바꾸지 말고 **[Next]를 계속 누르다가 [Install] → [Finish]** 하면 끝입니다.
...
2. zip 파일을 원하는 폴더에 **압축 해제**하고, 폴더 안의 **`start-windows.bat` 를 더블클릭**합니다.
   몇 초 뒤 브라우저가 자동으로 열립니다. (Linux는 터미널에서 `sh start-linux.sh` 실행 후
   브라우저에서 `http://localhost:8080` 접속)

3. 첫 실행 때 관리자(admin)의 **초기 비밀번호**가 검은 실행 창에 표시되고, 같은 내용이
   `data/initial-admin-password.txt` 파일에도 저장됩니다. 그 비밀번호로 로그인한 뒤 바꿔 주세요.
...
선행조건: **Node LTS(20.19+ 또는 22.12+)** 하나.
...
Releases에 자동 첨부합니다. zip은 완전 오프라인 실행 가능(prod `node_modules` 동봉 — 대상 서버에
Node LTS만 있으면 됨). 데모 데이터가 필요하면 저장소를 클론해 위 "5분 설치"를 이용하세요.

## 문서

- 설치(내부망 오프라인): [docs/INSTALL.md](docs/INSTALL.md)
- 운영 런북(재시작·백업·사용자·무결성·심사 D‑1 체크리스트): [docs/운영런북.md](docs/운영런북.md)

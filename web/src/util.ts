...
/**
 * 개정 사유 표시 문구 — 문항 PDF의 공식 개정 요약표에서 온 값이면 출처를 붙인다.
 * 서버가 본문 비교로 만든 폴백 문구('…(개정표 미기재)')에는 붙이지 않는다(자기모순 방지).
 */
export function revisionNoteLabel(note: string | null | undefined): string | null {
  if (!note) return null;
  return note.includes('개정표 미기재') ? note : `개정 요약표: ${note}`;
}

...
// 빌드 플랫폼의 zip 에는 그 플랫폼용 스크립트만 담는다(교차 노출이 비개발자를 혼란시킴 — 인수 리뷰).
// darwin 로컬 빌드는 검증용이라 둘 다 유지. packaging/ 폴더 사본에는 항상 둘 다 있다(IT 참고용).
const rootScripts =
...
for (const f of rootScripts) {
  ...
  if (f === 'start-linux.sh') fs.chmodSync(path.join(stageDir, f), 0o755);
}

// 5-2) 처음 읽어주세요.txt — 압축 해제 직후 무엇을 실행할지 안내 (UTF-8 BOM + CRLF: 메모장 호환)
const readmeFirstBody = [
  '우수검사실 인증심사 웹 바인더 — 처음 읽어주세요',
  '',
  ...(rootScripts.includes('start-windows.bat')
    ? [
        '[Windows] start-windows.bat 파일을 더블클릭하면 실행됩니다.',
        '  - 검은 창이 뜨고, 준비가 끝나면 브라우저가 자동으로 열립니다.',
        '  - 처음 실행하는 PC에서는 보안 검사 때문에 1~3분 걸릴 수 있습니다.',
        '    화면이 늦게 떠도 고장이 아닙니다 - 검은 창을 닫지 말고 기다려 주세요.',
        '  - 검은 창을 닫으면 프로그램도 종료됩니다.',
      ]
    : []),
  ...(rootScripts.includes('start-linux.sh')
    ? ['[Linux] 터미널에서 sh start-linux.sh 를 실행한 뒤 브라우저에서 http://localhost:8080 을 여세요.']
    : []),
  '',
  '처음 로그인: 아이디 admin, 비밀번호는 첫 실행 때 만들어지는',
  `data 폴더 안의 initial-admin-password.txt 파일 안에 있습니다.`,
  '',
  '다른 PC에서 접속하려면 실행 창에 표시되는 "같은 내부망에서 열기" 주소를 사용하세요.',
  '자세한 설치·운영 방법은 docs 폴더의 INSTALL 문서를 참고하세요.',
].join('\r\n');
fs.writeFileSync(path.join(stageDir, '처음 읽어주세요.txt'), '\uFEFF' + readmeFirstBody + '\r\n');

...
echo.
echo   웹 바인더를 시작합니다 - 준비가 끝나면 브라우저가 자동으로 열립니다.
echo   주소: http://localhost:8080   (안 열리면 브라우저 주소창에 직접 입력^)
rem config.json 에서 port 를 바꿨다면 위 주소의 8080 도 그 포트로 읽으세요.
echo.
echo   * 처음 실행하는 PC에서는 보안 검사 때문에 1~3분 걸릴 수 있습니다.
echo     화면이 늦게 떠도 고장이 아닙니다 - 이 창을 닫지 말고 기다려 주세요.
echo   * 이 창을 닫으면 서버가 종료됩니다.
echo.
start "" /b "%NODE_EXE%" packaging\open-when-ready.mjs
